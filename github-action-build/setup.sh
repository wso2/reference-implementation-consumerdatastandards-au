...
unzip $ARTIFACTS_DIR/wso2is-6.1.0.zip -d $TEST_HOME
unzip $ARTIFACTS_DIR/wso2am-4.2.0.zip -d $TEST_HOME
unzip $ARTIFACTS_DIR/wso2si-4.2.0.zip -d $TEST_HOME

echo '======================= Extracting Accelerator Artifacts to product home directory======================='
unzip $ARTIFACTS_DIR/wso2-obiam-accelerator-3.0.0.zip -d $TEST_HOME/wso2is-6.1.0
unzip $ARTIFACTS_DIR/wso2-obam-accelerator-3.0.0.zip -d $TEST_HOME/wso2am-4.2.0
unzip $ARTIFACTS_DIR/wso2-obbi-accelerator-3.0.0.zip -d $TEST_HOME/wso2si-4.2.0

# Extract the WSO2 IS Connector to the TEST_HOME directory
unzip $ARTIFACTS_DIR/wso2is-extensions-1.6.8.zip -d $TEST_HOME
...
NEW_JAR_SOURCE="$ARTIFACTS_DIR/org.wso2.carbon.extension.identity.authenticator.smsotp.connector-3.0.4.jar"
